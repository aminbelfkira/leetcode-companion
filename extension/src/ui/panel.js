...
// auto-contenus, aucune fuite CSS vers/depuis NeetCode.
...
.panel {
  ...
  --ink:#eff1f3; --mut:#8f959c; --nc:#48c78e; --ok:#48c78e;
...
.tag { color: var(--ok); font-size: 11px; font-weight: 600; letter-spacing: .02em; }
.close {
...
.saved { color: var(--ok); font-weight: 650; text-align: center; padding: 24px 0; }
`;

  let mounted = false;

  NCC.isPanelMounted = function isPanelMounted() {
    return mounted;
  };
  ...
  /**
   * data     : { title, ncDifficulty, submissionsInSession, minutesInSession }
   * callbacks: { previewDue(mode, feel), onSave(mode, feel), onDismiss() }
   */
  NCC.mountPanel = function mountPanel(data, cb) {
    ...
    const diffClass = String(data.ncDifficulty).toLowerCase();
    const minutesChip =
    ...
    const nextEl = panel.querySelector(".next b");
    const nextLine = panel.querySelector(".next");
    ...
    async function refreshPreview() {
      ...
      try {
        ...
        nextEl.textContent = due === null ? "—" : NCC.formatDueRelative(due);
      } catch {
        nextEl.textContent = "—";
      }
    }
