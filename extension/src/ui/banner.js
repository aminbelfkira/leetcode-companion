...
(() => {
  const NCC = self.NCC;
  const HOST_ID = "nccfsrs-banner-host";
...
.banner {
  ...
  border-bottom: 1px solid rgba(72, 199, 142, .4);
  color: #eff1f3;
  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
  font-size: 12px; line-height: 1.3;
  animation: nccfsrs-slide .18s ease-out;
}
@keyframes nccfsrs-slide {
...
button {
  ...
  background: none; border: 1px solid rgba(72, 199, 142, .5);
  border-radius: 6px; color: #48c78e;
  padding: 2px 8px; font-size: 11px;
  white-space: nowrap;
}
button:hover { background: rgba(72, 199, 142, .15); }
...
`;
  ...
  /**
   * info : { count, next: { slug, title } }
   * cb   : { onOpen(slug), onSnooze() }
   */
  NCC.renderBanner = function renderBanner(info, cb) {
    callbacks = cb;
  ...
  NCC.removeBanner = function removeBanner() {
    host?.remove();
    host = null;
    txtEl = null;
    callbacks = null;
