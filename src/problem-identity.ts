...
export function findExistingProblemId(
  ...
  for (const card of Object.values(cards)) {
    ...
    if (
      ...
      Object.values(card.sources).some((candidate) => candidate?.frontendId === problem.frontendId)
    ) {
      return card.id;
    }
  }
  ...
  const fuzzyTitle = Object.values(cards).filter((card) => titlesProbablyMatch(card.title, problem.title));
...
export function sourceFromDescriptor(
  ...
  return {
    ...
    difficulty:
      problem.difficulty === "Unknown" ? (previous?.difficulty ?? "Unknown") : problem.difficulty,
    lastSeenAt: now,
  };
}

export function problemUrl(card: ProblemCard, preferredPlatform?: "leetcode" | "neetcode"): string {
  const platform =
