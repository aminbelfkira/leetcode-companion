// Popup « Focus intelligent ». Lectures directes du storage, mutations via
...
const ICONS = {
  ...
  github: `<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="6" cy="6" r="2" /><circle cx="18" cy="6" r="2" /><circle cx="12" cy="18" r="2" /><path d="M8 6h4a4 4 0 0 1 4 4v1M8 6v4a4 4 0 0 0 4 4v2" /></svg>`,
  ...
  if (status.enabled && status.repository !== null) {
    const detail =
      status.lastError !== null
        ? `<span class="sync-state error">Synchronisation à vérifier</span>`
        : status.pendingCount > 0
          ? `<span class="sync-state pending">${status.pendingCount} en attente</span>`
          : `<span class="sync-state ok">Synchronisation active</span>`;
    return `
      <button class="sync-strip" data-options>
        <span class="sync-icon">${ICONS.github}</span>
        <span class="sync-copy">
          <span class="sync-title">${esc(status.repository.fullName)}</span>
          ${detail}
        </span>
        <span class="row-chevron">${ICONS.chevron}</span>
      </button>`;
